...
```
/boot/fpga.rbf  (symlink or direct copy)
```

U-Boot loads it via device tree overlay or explicit command. The Buildroot kernel is configured with `BR2_LINUX_KERNEL_EXT_SOCFPGA=y`, which enables the FPGA Manager.
